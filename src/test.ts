import { Mp3Downloader, Mp3Header } from './index';
...
(async () => {
  const urls: Array<string> = [
    ...
    'http://traffic.libsyn.com/shepodcasts/sp234_final.mp3?dest-id=191826',
  ];

  for (let i: number = 0; i < urls.length; i++) {
    console.log(urls[i])
    const mp3Downloader: Mp3Downloader = new Mp3Downloader(urls[i]);
    const data: Mp3Header = await mp3Downloader.getMp3Data();
